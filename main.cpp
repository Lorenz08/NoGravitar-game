...
int main() {
	...
	while (gioco.boolGame()) {
		...
		char moveSpaceshipUniverso = _getch();
		...
		if (planetEnter == true) {
			...
			while ((uno.pianetaCor(nav_SS.coordinate(true), nav_SS.coordinate(false))->uscitaPianeta(nav_Pianeta.coordinatePlanet(true), nav_Pianeta.coordinatePlanet(false), tastoPremuto)) && (gioco.boolGame())) {  //cpntrolla se la posizione della navicella è nella prima riga, in tal caso vuol dire che è uscita dal pianeta
				...
				tastoPremuto = _getch();    //si preme un tasto e in base ad esso il programma esegue diverse interazioni

				if ((tastoPremuto == 'q') && (uno.pianetaCor(nav_SS.coordinate(true), nav_SS.coordinate(false))->raggioTraenteUscente(nav_Pianeta.coordinatePlanet(true), nav_Pianeta.coordinatePlanet(false), raggio))) {  //controlla che il raggio non tocchi la superficie e se verificato per ogni tasto 'q' premuto esce un pezzetto di raggio
