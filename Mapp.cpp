...
void Mapp::setParametriGiocoUniverso(int fuel, int life) {
	...
	matrice[3][88] = 'E';
	matrice[3][89] = ':';
	matrice[3][90] = (char)migliaia + 48;
	matrice[3][91] = (char)centinaia + 48;
	matrice[3][92] = (char)decine + 48;
	matrice[3][93] = (char)unità + 48;
...
void Mapp::printMapp() {
	...
	for (int i = 0; i < x; i++) {
		for (int j = 0; j < y; j++) {
			...
			if (matrice[i][j] == '|') SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 12 );
			if (matrice[i][j] == 'Y') SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), FOREGROUND_RED);
			if ((matrice[i][j] == 'C') && ((j >= 1) && (j <= 77)))  SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 1);
			if ((matrice[i][j] == 'c') && ((j >= 1) && (j <= 77)))  SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 3);
			cout << matrice[i][j];
		}
		cout << "\n";
	}
