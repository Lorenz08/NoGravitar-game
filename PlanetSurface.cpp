...
PlanetSurface::PlanetSurface(Spaceship spaceship, ptr_listaBunker1& head1, ptr_listaBunker2& head2) {
	setMapp();
	setPlanetSurfaceParameters(spaceship.returnParameter(100), spaceship.returnParameter(10), spaceship.returnParameter(1000));
...
void PlanetSurface::setMappPlanetSurface(Spaceship & p, char n, bool& uscitaPianeta) {
	setPlanetSurfaceParameters(p.returnParameter(100), p.returnParameter(10), p.returnParameter(1000));
	/*for (int i = 1; i < 78; i++) {
		if (matrice[i][1] != '.') matrice[i][1] = ' ';
	}*/
...
void PlanetSurface::refreshBunker1(Spaceship & p, ptr_listaBunker1 & head1, int i) {
	...
	tmp1 = head1;
	while (tmp1 != NULL) {
		ptr_Bullet tmpSx = tmp1->b1->LPsx->listaProiettili;
		while (tmpSx != NULL) {
			tmpSx->xBullet--;
			tmpSx->yBullet--;
		...
		ptr_Bullet tmpDx = tmp1->b1->LPdx->listaProiettili;
		while (tmpDx != NULL) {
			tmpDx->xBullet++;
			tmpDx->yBullet--;
...
void PlanetSurface::refreshBunker2(Spaceship & p, ptr_listaBunker2 & head2, int i) {
	...
	tmp2 = head2;
	while (tmp2 != NULL) {
		ptr_Bullet tmpSx = tmp2->b2->LPsx->listaProiettili;
		while (tmpSx != NULL) {
			tmpSx->xBullet--;
			tmpSx->yBullet--;
			if ((matrice[tmpSx->xBullet][tmpSx->yBullet] == ' ') || (matrice[tmpSx->xBullet][tmpSx->yBullet] == '.')) {
		...
		ptr_Bullet tmpCentrale = tmp2->b2->LPcentrale->listaProiettili;
		...
		ptr_Bullet tmpDx = tmp2->b2->LPdx->listaProiettili;
		while (tmpDx != NULL) {
			tmpDx->xBullet++;
			tmpDx->yBullet--;
			if ((matrice[tmpDx->xBullet][tmpDx->yBullet] == ' ') || (matrice[tmpDx->xBullet][tmpDx->yBullet] == '.')) {
...
void PlanetSurface::refreshSpaceship(Spaceship & p, ptr_listaBunker1 & head1, ptr_listaBunker2 & head2) {
	ptr_Bullet tmp = p.LP->listaProiettili;
	int xx = 0;
	int yy = 0;
...
void PlanetSurface::eliminareSpariBunker1Distrutto(ptr_listaBunker1 & head) {
	ptr_Bullet tmpSx = head->b1->LPsx->listaProiettili;
	ptr_Bullet tmpDx = head->b1->LPdx->listaProiettili;
	while (tmpSx != NULL) {
	...
	while (head->b1->LPsx->listaProiettili != NULL) head->b1->LPsx->deleteBullets();
	while (head->b1->LPdx->listaProiettili != NULL) head->b1->LPdx->deleteBullets();
}


void PlanetSurface::eliminareSpariBunker2Distrutto(ptr_listaBunker2 & head) {
	ptr_Bullet tmpSx = head->b2->LPsx->listaProiettili;
	ptr_Bullet tmpDx = head->b2->LPdx->listaProiettili;
	ptr_Bullet tmpCentrale = head->b2->LPcentrale->listaProiettili;
	while (tmpSx != NULL) {
		matrice[tmpSx->xBullet][tmpSx->yBullet] = ' ';
		tmpSx = tmpSx->next;
	}
